...
export default function ClubPollsTab({
	...
	const filteredPolls = clubPolls.filter((poll) => {
		if (searchQuery.trim()) {
			...
			if (!matchTitle && !matchDesc && !matchOpt) return false;
		}
	...
	return (
		<main className="grow mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8 py-6 space-y-6">
			{/* Hub Header with Summary Metrics (Leaders & Officers Only) */}
			{canManage && (
			...
			<div className="p-4 rounded-2xl border border-border bg-surface shadow-2xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
				<div className="relative grow max-w-md">
					...
					<input
						...
						placeholder="Search club polls..."
						className="w-full pl-10 pr-4 py-2 rounded-xl border border-border bg-surface-secondary text-xs text-text-primary focus:outline-none focus:ring-2 focus:ring-primary/30 transition-all"
					/>
				</div>
				...
				{canManage && onOpenCreateModal && (
					<button
						...
						data-tour="polls-create"
						className="px-4 py-2 rounded-xl bg-primary text-white font-bold text-xs hover:bg-primary-hover transition-all flex items-center justify-center gap-1.5 shadow-sm shrink-0 cursor-pointer"
					>
						<FiPlus size={15} />
